...
Revision 4 replaces only the prior Windows icon bytes at the same resource path. The v2 brighter STUDIO header, splash bytes, and all presentation behavior are unchanged. The v4 icon preserves the complete composition at approximately 92% canvas fill; no individual PNG frames are required by the existing toolchain.
