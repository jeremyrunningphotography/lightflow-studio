    ...
    Func<IReadOnlyList<BrowserGridTile>, IEnumerable<BrowserFilterPredicate>> Values)
{
    ...
    public bool PreferPresets { get; init; }
    public bool DurationComparisons { get; init; }
...
internal static class BrowserFilterDescriptors
{
    ...
    // Exact raster sizes explicitly accepted for this iteration; identity remains integer dimensions.
    private static IReadOnlyList<BrowserFilterPredicate> CommonResolutions { get; } =
    [BrowserFilterPredicate.ForResolution(1280, 720), BrowserFilterPredicate.ForResolution(1920, 1080),
        BrowserFilterPredicate.ForResolution(2560, 1440), BrowserFilterPredicate.ForResolution(3840, 2160),
        BrowserFilterPredicate.ForResolution(4096, 2160)];
    ...
    [
        ...
        new(BrowserFilterField.Duration, "Duration", BrowserFilterEditorKind.Structured, tiles => new[] { 10d, 30d, 60d, 300d }
            ...
            .Select(n => BrowserFilterPredicate.ForMinimum(BrowserFilterField.Duration, n))) { Input = BrowserStructuredFilterInput.Duration, OfferSuggestions = false, DurationComparisons = true },
        new(BrowserFilterField.Resolution, "Resolution", BrowserFilterEditorKind.Structured, tiles => tiles.Where(t => t.PixelWidth is > 0 && t.PixelHeight is > 0)
            ...
            .Select(s => BrowserFilterPredicate.ForResolution(s.W, s.H))) { Input = BrowserStructuredFilterInput.Resolution, PreferPresets = true,
                Suggestions = tiles => CommonResolutions.Concat(Values(BrowserFilterField.Resolution, tiles)).Distinct() },
        ...
        State(BrowserFilterField.ReviewRangeState, "In/Out Range", "is set", "is not set"),
        State(BrowserFilterField.SubclipState, "Subclips", "exist", "do not exist"),
