...
/// <summary>Version 2 uses one row per Field: alternatives OR within a field, MatchMode between fields.
...
internal sealed record BrowserQueryIntent
{
    ...
    public static BrowserQueryIntent Deserialize(string json)
    {
        ...
        if (!document.RootElement.TryGetProperty(nameof(Version), out var version) || !version.TryGetInt32(out var number) || number is not (1 or 2))
        ...
        result.Validate();
        return result with { Version = 2, SearchText = "", Filters = WithSearch(result.Filters, result.SearchText) };
    }
    ...
    {
        if (Version is not (1 or 2) || !Enum.IsDefined(MatchMode) || SearchText is null || Filters is null ||
