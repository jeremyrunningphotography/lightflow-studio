...
internal enum DescriptionTransitionChoice { Cancel, Apply, Discard }
...
internal sealed class InspectorDescriptionField : InspectorObservable
{
    ...
    public InspectorDescriptionField(AssetDescriptionField field, IReadOnlyCollection<AssetDescription> values)
    {
        ...
        _text = IsMixed ? "" : first ?? "";
        _originalText = _text;
        _valueState = IsMixed ? "Mixed values · type to replace" : "";
    }

    internal void Discard()
    {
        _text = _originalText; _editedMixed = false;
        Changed(nameof(Text)); Changed(nameof(IsDirty)); Changed(nameof(ValueState));
    }
}
    ...
    Func<DescriptionConfirmation, bool> confirm) : InspectorObservable, IDisposable
{
    ...
    public bool CanEdit => _ready && !_loading && !_saving && !_confirming && _navigation == 0;
    ...
    internal IDisposable SuspendEditing()
    {
        _navigation++; NotifyState();
        return new NavigationLease(() => { _navigation--; NotifyState(); });
    }
    private sealed class NavigationLease(Action release) : IDisposable
    {
    ...
    public Task SetContextAsync(IEnumerable<Guid?> ids, bool player)
    {
        ...
        // Context owners obtain permission before replacing selection/presentation.
        if (HasDraft || _saving || _confirming) return Task.CompletedTask;
        _context = context;
        _player = player;
    ...
    public bool CanLeaveContext => !_saving && !_confirming;
    public DescriptionConfirmation PendingChanges => new(true, _revisions.Count,
        Fields.Where(f => f.IsDirty).Select(f => f.Label).ToArray());

    public async Task<bool> ResolveTransitionAsync(DescriptionTransitionChoice choice)
    {
        if (!CanLeaveContext) return false;
        if (!HasDraft) return true;
        if (choice == DescriptionTransitionChoice.Cancel) return false;
        if (choice == DescriptionTransitionChoice.Apply) return await ApplyAsync(transitionConfirmed: true);
        foreach (var field in Fields) field.Discard();
        _discardNotice = "Unapplied changes discarded.";
        Status = _discardNotice;
        NotifyState();
        return true;
    }
    ...
    public async Task<bool> ApplyAsync(bool transitionConfirmed = false)
    {
        ...
        try { patch.Validate(); }
        catch (ArgumentException exception) { Status = $"Descriptions were not saved. {exception.Message}"; return false; }
        if (count > 1 && !transitionConfirmed && !ConfirmAction(isApply: true)) return false;
        _saving = true;
