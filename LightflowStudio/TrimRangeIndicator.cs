...
internal sealed class TrimRangeIndicator : FrameworkElement
{
    ...
    public static readonly DependencyProperty DimOutsideProperty = DependencyProperty.Register(
        nameof(DimOutside), typeof(bool), typeof(TrimRangeIndicator), new FrameworkPropertyMetadata(false, FrameworkPropertyMetadataOptions.AffectsRender));
    ...
    protected override void OnRender(DrawingContext drawingContext)
    {
        ...
        if (DimOutside)
        {
            var shade = new SolidColorBrush(System.Windows.Media.Color.FromArgb(150, 2, 3, 5));
            shade.Freeze();
            drawingContext.DrawRectangle(shade, null, new Rect(0, 0, start, ActualHeight));
            drawingContext.DrawRectangle(shade, null, new Rect(end, 0, Math.Max(0, width - end), ActualHeight));
        }
