...
The provider-neutral result distinguishes a missing root, unavailable/unmapped root, missing child folder, access denial, invalid logical path, transient I/O failure, and invalid out-of-root provider output. Unavailable roots are rejected before filesystem enumeration and are never interpreted as deleted assets. Enumeration performs no fingerprinting, metadata probing, thumbnail generation, Catalog reconciliation, or monitoring; Issues #99–#101 own those later stages.
