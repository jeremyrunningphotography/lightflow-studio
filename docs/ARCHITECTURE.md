...
Issue #107 adds a UI-independent `BrowserNavigationSession` to the permanent Browser workspace. A Browser location is always a logical `RootId` plus normalized relative folder, never an arbitrary physical path. Each accepted navigation first uses `IMediaDiscoveryRefreshService` for authoritative Catalog reconciliation and then uses the #98 enumerator for the basic folder/file presentation. Unavailable/unmapped roots and provider-neutral enumeration failures become non-destructive workspace states. Back, Forward, Up, and Refresh retain logical history; Refresh does not add history. A per-request generation and linked cancellation token ensure rapid source changes are latest-request-wins and late results cannot overwrite the current folder.

The session and its basic entry list remain alive while the user visits other Lightflow workspaces, preserving Browser location and history. The WPF layer owns only input handling, loading/empty-state presentation, and collection updates; path containment, reparse-point rejection, discovery, and Catalog mutation remain behind existing services. Issue #107 deliberately does not add thumbnail presentation, selection, sorting/filtering/search, Player/Viewer presentation, Inspector/Color, or Browser-to-Encoding handoff; Issues #108–#112 extend this workspace.
