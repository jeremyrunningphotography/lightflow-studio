...
A follow-up mockup-driven revision (still #109) adds a persistent All/Images/RAW/Video `ToggleButton` row alongside `Filter ▾` — the common single-value case as one click, coexisting with (not replacing) the popup's stackable checkboxes for the same field. `BrowserQuery.WithOnlyFilter` replaces the entire media-type facet with exactly one predicate (the quick buttons' write path); `WithoutField` clears a facet entirely ("All", a no-op returning the same instance if the facet was already empty — the same `Assert.Same`-testable shape as `WithFilterRemoved`). `SyncBrowserQueryToolbarVisuals` derives the four buttons' `IsChecked` state from the distinct media-type values currently active: none active checks "All", exactly one value checks that button, and more than one (only reachable via `Filter ▾`) leaves all four unchecked, since that combination has no single-button representation — one more visual the same centralized sync method keeps from drifting. The search box gained a placeholder ("Search assets…") via an overlay `TextBlock` (`IsHitTestVisible="False"`) bound to the box's own `Text` through a new `StringEmptyToVisibilityConverter`, and a window-level `PreviewKeyDown` handler focuses it on Ctrl+F, scoped to only act while the Browser tab is selected and the toolbar is enabled (a folder loaded) so the shortcut never steals focus from another workspace. The status line (`BrowserStatusText`) moved out of the toolbar row into its own bar beneath the grid, so it stays with the media it describes instead of crowding the toolbar's controls.
