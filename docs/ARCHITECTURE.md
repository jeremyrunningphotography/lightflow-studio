...
Issue #108 turns the central surface into a virtualized media thumbnail grid without introducing any parallel enumeration, Catalog, Preview, or filesystem-watching path. `BrowserNavigationSession` already calls `IMediaDiscoveryRefreshService.RefreshAsync` at `DerivedWorkPriority.Visible` for every folder it loads; `BrowserFolderState` now also carries the `IDerivedWorkBatch` that call schedules, so the grid consumes the batch navigation already produced rather than resubmitting work. `BrowserGridModel` builds one `BrowserGridTile` per non-directory entry in the existing deterministic enumeration order, matches each tile to its stable Catalog `AssetId` using `IDerivedWorkBatch.Reconciliation.Items` (no separate `IMediaAssetService` lookups), and applies a generated thumbnail's resolved absolute path only for assets the batch reports with a `Succeeded`/`Current` thumbnail outcome, read once through `IPreviewStoreService.GetAsync`. Thumbnails update tiles in place; `BrowserGridModel.Populate` reuses existing tile instances by stable `RelativePathKey` across a non-destructive refresh so already-resolved thumbnails and selection survive it. Unsupported files never receive a Catalog asset or thumbnail and remain a stable placeholder tile; RAW and audio assets are valid Catalog assets that currently never receive a generated thumbnail either, since thumbnail generation remains scoped to `image`/`video` media types.

The grid is virtualized by grouping tiles into fixed-width rows (`BrowserGridLayout`) and letting a native `VirtualizingStackPanel` recycle only the realized rows; each row's own tile `ItemsControl` is small and unvirtualized by design. `MainWindow` recomputes the column count from the media canvas's available width on layout changes, so both application resizing and dragging the Locations splitter reflow the grid without introducing horizontal scrolling. Selection (`BrowserGridSelection`) is keyed by stable `RelativePathKey` rather than visual containers, so it survives recycling and refresh; single-click, Ctrl-toggle, Shift-range, Select All, and clear-on-empty-canvas are implemented directly against tile indices rather than relying on a `Selector`'s built-in multi-select model, since tiles are nested two levels below the virtualized row list.
