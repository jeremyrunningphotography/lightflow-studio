...
The storage coordinator exclusively owns restore lifecycle and session quiescing. It validates the selected backup and Catalog identity before mutation, creates a validated recovery backup of an available current Catalog, disposes the session/clears SQLite pools, stages the selected database through SQLite online backup, preserves the displaced live files, and validates again after same-directory replacement. Failure attempts rollback to the displaced Catalog; if rollback itself fails, diagnostics identify the preserved recovery path rather than creating an empty Catalog. A corrupt/unreadable startup remains nonfatal for Encoding and exposes existing backups in the minimal Settings recovery UI. Restore never reads, copies, deletes, or relocates Previews.
