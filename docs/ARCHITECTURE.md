...
`MediaPlaybackView` owns a disposable Lightflow presentation lease. Closing an editor clears the view content and explicitly releases the Flyleaf host's renderer attachment before releasing the global playback lease. A later editor reattaches that same backend host to its current player, preserving one global playback session while preventing a closed WPF view from retaining the renderer.
