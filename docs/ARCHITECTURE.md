...
active surface; responsive clamping preserves that preference when a narrow window temporarily reduces space.
Opening, closing, or resizing the panel never rebuilds Browser or Player. Jobs remains globally owned and uses the same host.
...
- **Preview generation is independent from live playback.** Rebuildable Browser Previews and future Preview derivatives do not depend on the Windows Player implementation merely because both can render Color.
...
both live presentation and native snapshots. An empty/default pipeline and momentary Original bypass use the same renderer-targeted GPU draw
...
Camera → Creative without reopening or interrupting playback. A new asset defaults to Original with independent
Camera and Creative `No LUT` selections. Those selectors are the persistent controls and remain available when the
Player controls publish; choosing a LUT makes Color active and clearing both stages returns to Original. Hold-C is a
momentary renderer bypass only. Neither selector interaction starts or waits on discovery.

### Browser Color materialization and Encoding

Catalog Color is durable per-asset user intent: optional Camera/technical and Creative assignments remain keyed to
stable `AssetId` values, and current active state is derived from any non-default assignment. The Browser-to-Encoding capability handoff is
the materialization point for that intent. Alongside the independent saved-range snapshot established by #144,
each Browser input receives an engine-neutral `MaterializedColorPipeline` containing the derived active state
...
preference set, atomically persists `AppSettings`, applies Export defaults, refreshes only changed LUT collections and
their live Player selectors, and re-evaluates processing dependencies. Storage relocation, Catalog recovery, and
Preview maintenance retain their own explicit transactional actions and never masquerade as ordinary preference saves.
Entering or leaving Settings changes only `ShellDestination`; it does not reconstruct Browser/Player state or disturb
the global Jobs scheduler.
...
Issue #138 adds a separate tile-facing projection for durable user-authored Catalog state. `BrowserAssetState` is an extensible flags vocabulary rather than a trim-specific tile property; `CatalogBrowserAssetStateStore` projects Catalog-owned facts without changing thumbnail generation or moving rebuildable Preview facts into the Catalog. The Browser reuses the reconciliation batch's stable `AssetId` values and reads state for those IDs in bounded SQLite batches, then updates existing tile instances in place. Player saves publish committed semantics without referencing Browser presentation controls; the Browser host maps those changes into flags after commit. Per-asset revisions prevent an older asynchronous folder-state read from overwriting a newer committed change without allowing an unrelated or previous-scope save to invalidate the rest of the folder's projection. Thumbnail files and Preview rows remain untouched throughout. #138's original single aggregate diamond presentation was superseded by #161's state-specific shared vector vocabulary and three presentation modes.

Issue #205 adds one more Catalog-owned per-asset user intent without making Preview pixels authoritative: an optional exact decoded media timestamp selects the preferred Browser Preview frame. The Player may commit or reset that timestamp only while video is already paused, using the displayed decoded timestamp (including a retained backward-stepped frame) without seeking, FPS-derived addressing, or implicit pause. The existing Color-aware thumbnail generator reads the intent after entering its generation gate, uses it instead of automatic representative-position selection, and folds timestamp plus Catalog revision into the ordinary Preview visual identity. It rechecks that combined Color/frame identity before publishing, so older asynchronous work cannot overwrite a newer choice. Reset removes the intent and regenerates only that asset through the same automatic Preview path; Catalog stores no image bytes or thumbnail path.

Issue #161 extends that projection with durable saved-Subclip presence (`EXISTS`/distinct `AssetId` rows in `Subclips`) in the same bounded batches as Review Range and Color. Successful first-create/final-delete mutations publish presence through the Player host boundary only after the Catalog commit; rename is deliberately silent. Every live durable-state mutation owns exactly one flag: Browser and Player Color, Player Review Range, and Player Subclips all pass their post-commit truth through MainWindow's shared single-flag publication boundary, which advances the per-asset revision before updating that flag in place and therefore cannot erase unrelated state. Only a bounded authoritative Catalog projection may replace the complete mask, and its captured revision is rejected if any newer live commit for that asset occurred while the read was in flight. `BrowserViewMode` is presentation-only state propagated across retained tiles without repopulation or query/selection changes. Preview makes the Preview surface the tile's only persistent content: no durable icons or Info frame, and the shared filename container is genuinely collapsed so it reserves no footer height (the unchanged outer tile tooltip/automation label still identifies the asset). Info reserves a quiet upper future-state frame, insets the Preview, places only active state vectors in a lower frame, and keeps the filename; Hybrid keeps the full Preview, overlays the same compact active-state cluster, and also keeps the filename. The shared bracket, stacked-range, and segmented-wheel vectors remain shape-distinct independent of selection color. One global view choice persists in tolerant `WorkspaceLayoutState` beside Preview size and follows the user unchanged across folder navigation and scope changes; it is never keyed by Catalog/folder identity and never touches reconciliation, filesystem enumeration, or Preview cache identity. Info and Hybrid share one dynamic icon-item spacing resource: compact at Small and more open at every larger size, while collapsed inactive presenters naturally close the row.

Issue #148 extends that separation to Color-aware visual derivatives. Catalog `AssetColorIntent.ColorIdentity` remains durable intent and never enters `PreviewSourceIdentity`, so LUT changes invalidate visual artifacts without reprobeing technical metadata. Preview schema 2 stores a distinct visual identity for both `Thumbnail` and `StandardPreview` artifact slots; this is a shared persistence/identity contract, not a claim that a separate `StandardPreview` renderer currently applies Color. The implemented Browser visual-generation path is the thumbnail-sized Browser Preview renderer, and #148 makes that path Color-aware. Its canonical Original identity is used when both stages are clear, while Camera-only, Creative-only, and ordered Camera→Creative intent produce their committed Color identity. Artifact paths include source plus visual identity, and generation re-reads both source and committed Color immediately before publication so stale work is discarded rather than overwriting newer intent. Video Browser Previews reuse the existing FFmpeg thumbnail renderer with ordered `lut3d` filters; unavailable LUTs retain honest prior pixels, persist a retryable failed state, and are never marked current. Startup LUT discovery and Browser restoration are independently asynchronous: after both cached LUT stages finish their initial refresh, the current Browser reconciliation is resubmitted once at visible priority (or audited when a later scope applies), ensuring an earlier attempt that honestly failed against not-yet-published LUT snapshots is retried and persisted Original→Color or Color-A→Color-B mismatches cannot wait for an in-session assignment event. Automatic/background and Color-triggered requests are ensure-current operations, while the explicit Regenerate Previews command remains force rebuild. A shared per-asset gate serializes overlapping UI batches; after that gate and the bounded renderer gate are acquired, ensure-current work resolves the latest committed Color and rechecks source, generator, and visual identity before taking an activity lease. Thus queued Camera→Creative bulk passes coalesce naturally: obsolete in-flight output still fails supersession, not-yet-started assets render only the latest pipeline, and later requests already satisfied by that render return Current without FFmpeg or a working-indicator flash. The existing bounded generation path publishes transient per-asset activity for genuinely rebuilding initial, background, Color-triggered, and manual work; tiles keep their current bitmap or placeholder under a small indeterminate indicator, cleared by lease disposal on every success, failure, cancellation, or supersession. Multi-asset regeneration reports each completed artifact at the shared batch boundary immediately after that asset's generation lease ends, so the Browser applies successful Preview pixels per tile without waiting for later assets while failures remain isolated and honest. `BrowserAssetState.Color` projects committed intent immediately through #138's bounded Catalog read and uses a compact six-hue segmented color-wheel marker with a gray unselected border and black selected border, independent of both artifact pixels and transient working state. The user-facing product term for these Browser images is Preview/Previews; established internal thumbnail generator/path contracts remain technical implementation names. Manual Regenerate Previews is a compact refresh icon immediately left of the Grid-only Preview-size controls in the Browser status/presentation area, targets the applicable selected image/video assets when selection exists, otherwise every applicable asset in the grid model's authoritative unfiltered current/effective recursive scope; folder-wide operations above 50 assets require confirmation and all targets still flow through the same bounded generator.
...
`Filter ▾` is a `ToggleButton` opening a `Popup` (mirroring the existing `RequirementHelpButton` pattern, but `StaysOpen="False"` for standard click-outside-to-dismiss dropdown behavior rather than a hover tooltip). `MainWindow.ApplyBrowserQuery(Func<BrowserQuery, BrowserQuery> transform)` is the single place that calls `BrowserGridModel.SetQuery`, and always finishes by calling `SyncBrowserQueryToolbarVisuals`, which reflects the model's current `Filters` onto the removable-chip `ItemsControl` (each chip is a real `Button`, not a bare clickable `TextBlock`, so removal stays keyboard-reachable and -activatable), quick media-type buttons, advanced-clear state, and sort-direction glyph. The chip row (and the space it would occupy) is `Visibility.Collapsed` whenever no advanced predicate is active, keeping the everyday toolbar a single quiet row.

A follow-up mockup-driven revision (still #109) adds a persistent All/Images/RAW/Video `ToggleButton` row alongside `Filter ▾`. Each media-type button is an **independent toggle**, not a mutually-exclusive pick: clicking an inactive button adds that predicate (`WithFilterAdded`), clicking an active one removes it (`WithFilterRemoved`) through the guarded `ToggleBrowserMediaTypeFilter` helper. `WithoutField` clears a facet entirely ("All", a no-op returning the same instance if the facet was already empty — the same `Assert.Same`-testable shape as `WithFilterRemoved`). `SyncBrowserQueryToolbarVisuals` checks "All" only from there being zero active media-type predicates, never from every individual button happening to be checked: "no explicit filter" and "every type explicitly selected" render the same tiles but are deliberately kept as two distinct, faithfully-preserved selections rather than normalized into one. `MainWindow.InitializeBrowserQuickFilterButtons` (called once from the constructor) generates one button per `BrowserGridModel.PresentableCategories` — the same list `IsPresentable` itself is built from — so the toolbar can never silently omit a category the grid actually presents; only "All" (not a media type) is hand-declared in XAML. Because the permanent buttons communicate the complete media-type facet, #210 removes Media Type from the advanced dropdown and `SyncBrowserQueryToolbarVisuals` excludes it from the active-filter chip row. The search box gained a placeholder ("Search assets…") via an overlay `TextBlock` (`IsHitTestVisible="False"`) bound to the box's own `Text` through a new `StringEmptyToVisibilityConverter`, and a window-level `PreviewKeyDown` handler focuses it on Ctrl+F, scoped to only act while the Browser tab is selected and the toolbar is enabled (a folder loaded) so the shortcut never steals focus from another workspace.

Issue #210 fills the extension seams #109 reserved without changing the query architecture. `BrowserFilterPredicate` now represents high-value normalized Preview facets (camera make+model, lens, capture-date range, minimum duration, exact resolution, and exact frame rate) plus durable Catalog facets (Color applied/Original color, Camera LUT presence, Creative LUT presence, saved working range, and saved Subclips). Same-field alternatives still OR and different fields still AND; active advanced predicates use the existing removable chips. Media Type exists only in the persistent toolbar, while the advanced dropdown gives Camera, Lens, Capture Date, Duration, Resolution, Frame Rate, and Lightflow State separate scannable groups plus an advanced-only Clear all filters action. Clear all retains toolbar Media Type and Search intent.

Advanced-choice discovery is contextual but non-recursive. `BrowserQueryEngine.ApplyAdvancedFilterContext` projects `_allTiles` through only current Media Type predicates and text search; it deliberately ignores every advanced predicate. Camera, lens, resolution, frame-rate, duration-threshold, and state presentation is then derived from that already-resident/hydrated set. Descriptive facets expose checkable predicates only when multiple known values can narrow the context; exactly one known value instead appears as quiet, non-selectable context, with coverage stated as all contextual items or only the known subset when other metadata is pending. Lightflow State remains a stable workflow-control group: its choices show counts computed only from tiles with an applied authoritative Catalog projection, and zero-count choices remain visible but disabled. An already-active zero-count choice stays enabled so it can be explicitly removed. Active predicates are never rewritten or removed when scope/search makes them temporarily unmatched: their chips remain explicit until individually removed or cleared through the deliberate advanced-only action. Duration uses compact creator-oriented thresholds that actually split the contextual duration set, and capture date accepts an inclusive optional-ended range. This remains transient `BrowserQuery` intent suitable for later #74 Smart Collection persistence; #210 adds no query persistence or second expression language.

Frame-rate filtering has one Browser-only canonicalization boundary: `BrowserFrameRate.Canonicalize`. Precise probed frame rate remains unchanged in authoritative Preview metadata and on the hydrated tile. Discovery, predicates, matching, chips, and single-value information all call the same function. The nearest standard nominal rate among 23.976, 24, 25, 29.97, 30, 50, 59.94, and 60 is selected only within 0.5% of that nominal value; overlapping neighborhoods deterministically choose the nearest nominal rate. Genuinely nonstandard positive rates remain distinct at the Browser's 0.001-fps presentation/query precision, while missing, non-finite, and non-positive values remain unknown and never form a bucket.

The hydration boundary remains explicit. `BrowserTechnicalMetadata` is a Browser-resident projection of #70/#91 normalized `PreviewRecords.MetadataJson`; `PreviewStoreService.GetManyAsync` reads requested Preview rows in bounded SQLite batches, replacing the earlier per-asset lookup loop, and streamed metadata changes share #109's roughly 800 ms coalesced query reapplication. `BrowserAssetQueryState` enriches #138's existing bounded `CatalogBrowserAssetStateStore` projection with Camera/Creative stage presence and Subclip count while retaining `BrowserAssetState` flags for presentation compatibility. Color, range, and Subclip services remain authoritative; committed live changes advance the existing per-asset revision, apply the coarse presentation flag immediately, then refresh that asset through the same Catalog projection so stage/count predicates stay current. Filter edits themselves are pure `BrowserQueryEngine.Apply` operations over `_allTiles`: no Preview/Catalog calls, source resolution, or synchronous probing occur per tile or per edit, and recursive scopes use the identical candidate-set → resident projection → query pipeline.

Missing semantics are deliberately not truthy defaults. A tile has separate `MetadataApplied` and `AssetStateApplied` hydration facts. A missing/not-yet-current Preview field matches neither an exact/threshold technical predicate nor a fabricated `0`; Catalog-backed negative predicates such as Original, no saved range, and no Subclips match only after the authoritative Catalog projection has applied, so a pending state read is not confused with a known negative. A current normalized metadata record may legitimately contain only some optional fields; each predicate requires its own value. This distinction stays entirely in Browser-resident projection state and does not create durable denormalized truth.
...
`BrowserLocationRestoration.RestoreAsync` is the sole Browser-restoration orchestrator and drives only the existing `BrowserNavigationSession.NavigateToRootAsync`/`NavigateToPathAsync` methods — the same calls a manual root click or direct-path entry already makes — so restoration adds no parallel enumeration/reconciliation/monitoring path. An offline or unmapped saved root resolves through `NavigateToRootAsync`, which already preserves and surfaces the honest `RootUnavailable` state a manual click on that root would show. An online root walks `AncestorRelativeFolderCandidates` — the saved folder, then each ancestor up to the root itself — calling `NavigateToPathAsync` for each candidate in turn; a missing/renamed folder fails at `BrowserLocationResolver`'s cheap existence check before ever reaching Catalog reconciliation, so only the one candidate that actually resolves triggers real reconciliation work, giving a deleted folder graceful, side-effect-free fallback to its nearest still-valid ancestor. `MainWindow.RestoreBrowserLocationAsync` runs this once after Locations storage entries are populated (so an offline root already has a tree node to show its unavailable state against) and feeds a successful result through the same `ApplyBrowserState`/`RevealBrowserTreeAncestorsAsync` path ordinary navigation uses; any outcome that resolves nothing leaves the Browser in its normal default empty state rather than blocking startup. The original #121 did not restore media selection; #247 supersedes that policy through the workspace continuation section described below.

Issue #247 extends the same per-user `WorkspaceStateService` to schema version 2. `Continuation` stores the existing typed `BrowserQuery` and query lock, logical expanded folder identities, shared sidebar offsets, AssetId selection/Shift anchor, a top-asset/within-row grid scroll anchor with pixel fallback, and optional Player continuation. Window/layout/Collection persistence remains in its existing sections. Final #124 recursive-root configuration remains Catalog-owned; normal navigation derives Include Subfolders rather than reading the obsolete workspace toggle. JSON section recovery isolates malformed Player data from otherwise valid Browser/window/layout state; writes retain the same atomic replacement and two-second debounce/shutdown lifecycle. Input schedules a debounced snapshot of current domain state, never per-frame or per-pixel disk writes.

`MainWindow.Workspace.cs` hydrates query/presentation intent before the first visible scope. Catalog-known content still uses #131's normal early presentation and Preview/state readers; selection intersects the restored result set. The existing pixel-virtualized rows provide extent/row metrics for content-anchored scroll restoration when layout becomes valid. `WorkspaceTreeRestoration` materializes only recorded branches and their required prefixes through `IMediaFolderEnumerator`, without Browser navigation, media reconciliation, or recursive descendant walks. Individual missing branches are skipped; saved flags restore collapsed ancestors as well as unrelated expanded branches. Startup reveal does not assign focus or scroll over the saved viewport.

Both normal Browser opens and continuation use `OpenResolvedBrowserPlayerAsync` and the same `PlayerViewerHost.OpenAsync`. Continuation resolves AssetId through `IMediaAssetService`, so current root/path relocation remains authoritative. Missing media falls back to the surrounding Browser; unavailable roots retain the host's existing honest unavailable presentation. For video, the existing normalized display timestamp is clamped to current duration and sought while paused before attaching the media surface. The host restores #199 Fit/pixel zoom, clamped pan, Loop, speed, source-valid rational cadence, mute/volume, and surviving Subclip selection through their existing owners. No range/Subclip/Color intent is copied into workspace state. Fullscreen, autoplay, input/focus/drag state, overlays, animations, and unfinished dialogs are excluded.

A revocable `WorkspaceRestorationRequest` supplies cancellation across navigation, folder materialization, identity resolution and Player opening. Actual mouse/key/wheel input and normal navigation revoke startup intent before their own actions; every delayed continuation publication checks cancellation, with normal Browser/Player generations remaining authoritative. The initial saved Player presentation is prepared before the window appears, avoiding a visible populated Browser-to-Player transition. Queued Browser scroll restoration is also discarded by new Browser input after Player startup has completed; image decode checks cancellation before publishing its surface. Inventory and final acceptance validation are recorded in `docs/performance/workspace-247.md`.
...
**First Location expansion (#281)**: expansion resolves missing Catalog identity before listing children, so a never-selected Location or descendant can open immediately. Already-materialized nodes are reused; one pending load per node coalesces rapid collapse/re-expand. Completion preserves current expansion intent and ignores detached/remapped nodes or nodes that navigation has already materialized. Unavailable locations/listings collapse back to a retryable state. No selection, Browser query, discovery reconciliation, Delete or drag/drop policy is invoked by expansion. `BrowserTreeExpansionLiveInteractionTests` exercises the actual disclosure toggle, selection independence, row input, keyboard expansion, descendant lazy loading, stable children, drop-highlight independence and persisted restoration.
...
Issue #244 confines Browser asset drag initiation to `BrowserGridRows`; the shared workspace ancestor retains drag feedback but cannot initiate tile drags from Player, Inspector, or other chrome. `SetBrowserPresentationMode` retires the tile origin and deferred single-selection gesture before switching visibility, while preserving Browser selection/query/scroll context. A new mouse-down, mouse-up (including handled releases outside the tile), grid capture loss, window deactivation, and close also retire the gesture. A consumed origin carries a generation checked after asynchronous file-source resolution and the initial adornment render, so a release or rapid presentation round trip cannot revive an old drag. Tile initiation owns no mouse capture and never releases capture belonging to Player controls. Native file/Collection payloads, drop destinations, and Player command/keyboard ownership remain unchanged.
...
Double-click, Enter, and the Browser context menu Open command enter the shared Player. Enter/Open choose the first selected asset in Browser order. Double-clicking a selected member preserves the deliberate multi-selection through its ordinary first-click selection collapse. Issue #111 defines the captured review set and return-selection behavior below.

Issue #111 captures a `PlayerReviewSet` from the already ordered Browser projection. A single-open review includes compatible video, still, and RAW results in the current folder or Collection after its existing query/filter/sort. A multi-selection includes only its compatible selected subset. Membership and order stay fixed for the open review; stable AssetId is authoritative, and each navigation resolves its current Catalog mapping. Each thumbnail binds the original notifying Browser Preview projection. There is no second query model, thumbnail cache, decoder, or neighbor predecode.

Ctrl+Left/Right, Previous/Next Asset buttons, and filmstrip selection all call `SelectReviewAssetAsync` through the same traversal path. Plain Left/Right keep frame stepping, including filmstrip focus; text entry retains ownership of its keys. Traversal never wraps. Requested selection advances synchronously, cancels the previous review/source request, and increments the host generation before path resolution. The old source pauses and releases its existing playback lease/presentation before the destination opens through `PlayerViewerHost.OpenAsync`. New videos remain paused at their own durable #133 In (source beginning when unset); no previous playhead is passed. Stale resolution, source loads, classifications, and queued playback callbacks cannot publish into a newer generation. Missing members use the existing unavailable presentation and can still be traversed past.

The compact horizontal filmstrip uses a recycling virtualized ListBox and coalesced current-item reveal. Previous/Next controls flank it. The toggle sits between timestamp and frame controls; hiding collapses the strip's layout space and exposes the count and compact navigation beside the toggle. Review membership/current item survive hiding. `WorkspaceLayoutState.PlayerFilmstripVisible` defaults true and participates in #247 capture/restore. Restoring Player reconstructs a review from the restored Browser projection/selection and current AssetId (appending the current asset if absent). The existing interrupted-session position is restored only for that current source; subsequent traversal follows paused-at-In. No per-asset playhead history or serialized duplicate review set is introduced. #199 fullscreen collapses all filmstrip chrome regardless of this preference and remains media-only. See `docs/performance/player-111.md` for coverage and acceptance evidence.
...
Opening the Player retains the Browser scope, query, sort, tiles, selection snapshot, and scroll position. Returning from a selected-subset review restores the opening multi-selection. Returning after traversal of a single-open result set selects the current asset and reveals its Browser row; without traversal, the opening selection/scroll remain. A genuine Browser scope change returns to Grid through the existing source teardown.
...
`MediaAssetColor` is the single durable per-asset Color intent. Camera and Creative `LutId` assignments are the
current editable state: either assignment means active; no assignments means Original. The schema-version-6
`ColorEnabled` column is retained temporarily for database compatibility, but reads ignore it as product truth and
stage writes normalize it to assignment presence (deleting an all-default row). This safely interprets legacy
Off-with-LUT rows as active and legacy On-with-no-LUT rows as Original without migration churn. Player selectors
read/write assignments asynchronously without initiating LUT discovery; hold-C remains presentation-only.
...
`MediaAssetColor` stores precious per-asset Color intent against stable `AssetId` as typed optional Camera/technical then Creative LUT references. `IAssetColorStore` provides single-stage and transactional bulk assignment mutation without source-media changes; clearing one stage preserves the other, and clearing both returns to Original. `AssetColorIntent.ColorEnabled` is a compatibility-named derived projection of `HasColor`, leaving one extension point for future native controls to participate in the same any-non-default rule. `AssetColorIntent.OrderedPipeline` is the engine-neutral Camera → Creative order later Player, Browser, and Encoding work consume. `ColorIdentity` hashes the versioned derived state, ordered LUT IDs, and immutable content hashes, so a presentation-affecting change alters visual identity while LUT renames and unrelated Catalog metadata do not. Rendered derivatives remain rebuildable Preview data and are intentionally not produced or invalidated here.
...
Export adds an explicit policy/materialization boundary. Job-global `EncodingJobOptions` may carry user policy, while each item stores `MaterializedExportSettings`: resolved supported video codec/container, source cadence/dimension traits, typed source-audio preference plus deterministic AAC fallback, and the final content-addressed Camera → Creative pipeline. Planning never chooses one representative file for a heterogeneous batch. FFmpeg consumes only the resolved item settings, and the complete record participates in output identity and serialized History/runtime recovery. The optional item field preserves compatibility with older JSON, which is interpreted through an isolated legacy adapter and is never re-probed merely to reinterpret an old Same-as-Source choice.

Encoder availability is also typed rather than Boolean. NVIDIA NVENC is implemented only when both H.264 and HEVC one-frame execution probes succeed against the selected packaged FFmpeg and current machine/driver; a service caches that initialization result. CPU, AMD AMF, and Intel Quick Sync remain explicit `NotImplemented` capabilities rather than misleading enabled choices.

File concurrency is bounded from 1 through 8, with 2 as the initial default; 1 preserves sequential behavior. Workers receive preplanned items and cannot derive names or read mutable Browser/Player configuration. Results and snapshots retain plan order regardless of completion order. Per-item progress is isolated and aggregate progress uses the existing work weights, is monotonic for a fixed plan, and counts completed work permanently. ETA is derived only after observed active throughput exists and is unavailable while pausing/paused or when work is indeterminate.
...
The #111 seek refinement opts into `SeekAccurateFromKeyframe` in the pinned package: seek backward at the target, decode to the existing accurate-frame tolerance, and retry the legacy three-second preroll if the frame is missing or overshoots. The same decoder and native seek lock own both attempts. The service publishes the settled decoded timestamp before awaiting audio restart, remaining in Seeking until resume completes. Slow stop/native-seek/audio-restart stages are recorded in the Activity Log for diagnosis; no frame-number approximation or extra playback engine is introduced.
...
Issue #54 can consume playback, actual displayed timestamps, and the Lightflow view without learning Flyleaf types. Issue #55 remains responsible for translating selected timestamps into FFmpeg's inclusive/exclusive range semantics. Issue #110 is this section's "future media browser" made real: `PlayerViewerHost` repeatedly transfers the one global playback lease exactly as anticipated here, issuing A → B → C load requests as the user opens different Browser assets, with only the latest generation ever allowed to publish state, frames, or audio — see the Browser Player/Viewer paragraphs above for the consumer-side detail. Issue #199 adds transient review options through that existing backend boundary: Flyleaf `Player.Speed` owns elapsed-time rate, and the bounded FFmpeg audio companion follows with bundled `atempo` processing. Source opens reset speed to 1×. The existing decoder's deterministic every-Nth-frame selection remains in use for exact divisors. Other lower canonical rates use an optional decoded-PTS selector in the pinned Flyleaf fork, with exact rational target slots and unchanged source timestamps/clock. The shared canonical rate definitions preserve integer and 1000/1001 fractions; the menu filters them against detected source FPS. Paused frame inspection restores unthinned source frames. No Catalog or Export intent is changed. No playback-quality mode is exposed because a measured general-purpose cheaper backend contract has not been established. Evidence and limitations are in `docs/performance/player-199.md`.

`MediaPlaybackPresentation.InputSurface` lets Player claim its native Flyleaf Window input without exposing Flyleaf types to the host. Claiming disables that consumer's backend default bindings; the Player-owned `PlayerSurfaceInput` routes single click, double click, pan, Ctrl+wheel, and keyboard input to existing Player commands. Single clicks execute on release without a timer. The first click in a double-click performs its normal toggle; the second changes fullscreen without another playback toggle. Drag/capture cancellation prevents an unfinished gesture from clicking. WPF still/retained-frame surfaces use the same gesture owner. Chrome and the shared Right Panel remain outside media-surface semantics; #244 BrowserGridRows drag initiation is unchanged.

`PlayerFullscreenPresentation` temporarily presents the same Player control in a root-spanning overlay slot while retaining the same Window content, attached shell, original parent, and window state. Exit hides the Player and native child while restoring the original parent/window, restores header and transport in that normal host, arranges the layout, and then shows the Player. This avoids exposing an intermediate fullscreen native rectangle over normal chrome without reopening media or transferring the playback lease. Normal Player header/transport and shell chrome are removed from fullscreen layout. Transient centered Play/Pause feedback, a first-entry Esc hint, and a pointer-revealed exit button overlay the entire media viewport. Video uses Flyleaf's existing transparent overlay HWND with software rendering isolated to that WPF layer (the native video renderer remains unchanged); still/retained frames use the same WPF overlay inside their media grid. MainWindow exits fullscreen before saving closing geometry. Fit/percentage zoom and bounded pan are transient host display state: native video uses the existing renderer viewport, and WPF still/retained frames use equivalent transforms. Pixel inspection accounts for source dimensions and WPF DPI. Loop reads the existing PresentedRange (selected Subclip before working range), or full source duration, and uses the shared Pause/Seek/Play path; backend end notification handles full-source wrap. Source release cancels gestures and resets all review controls.
...
`ICatalogRecoveryService` remains the provider-neutral recovery boundary. #272 replaces routine automatic startup backups with optional explicit backup-on-close (enabled by default) and a profile-owned destination. `CatalogMutationLifecycle` closes admission and drains complete logical mutations, including multi-transaction commands and background reconciliation, before a SQLite-safe snapshot. Cancellation or continuing after failure reopens admission; successful backup-to-exit retains quiescence through shutdown. The staged user copy receives full recovery validation and identity/schema checks before atomic publication, without a redundant source full scan. User-requested copies have a distinct `LightflowCatalog-User-v{schema}-{timestamp}.db` convention and are never automatically deleted. Required migration and pre-restore safety copies keep their existing managed path and retention. The writer inventory, operation ownership, cancellation boundaries, preference migration, and acceptance paths are documented in [Catalog exit backup #272](catalog-backup-272.md).
...
Recovery SQLite connections use normalized Windows extended paths so staging, journal, and final backup paths do not inherit the packaged host's legacy MAX_PATH restriction. The public paths, same-directory publication, and ordinary Catalog storage policy remain unchanged. Restore accepts cancellation before installation starts, then completes installed validation or rollback without cancellation. Packaged boundary checks run inside the application executable because VSTest's manifest can mask this native path restriction. See [#318 boundary evidence and safety contract](validation/ISSUE318_BACKUP_PATHS.md).
...
Issue #212 establishes manual organization as precious Catalog data through three distinct schema entities. `CollectionSets` form an arbitrarily nested, cycle-checked hierarchy and never contain assets; `Collections` have optional Set placement and contain logical membership; `CollectionAssets` relates a Collection to stable `AssetId` without duplicating media. #213 supersedes #212's separate same-kind presentation order: migration 11 redefines each Set/Collection `Ordinal` as one dense zero-based mixed sibling sequence at its parent. Existing version-10 data migrates deterministically in its previously visible order (Sets by their old ordinal, then Collections by their old ordinal); cross-table insert/update triggers prevent either kind from claiming the same mixed position. Creation, deletion, reparenting, arbitrary drag reorder, and name sort update that complete mixed sequence in immediate transactions, with complete-list entity revision checks for reorder. Membership retains its independent dense order within its Collection. Every mutable row has optimistic revision and fixed UTC timestamps. Set reparenting checks ancestry inside the same transaction. Set deletion is leaf-only with restrictive hierarchy foreign keys; Collection deletion explicitly removes only membership plus the Collection and never source assets. Membership references assets with `ON DELETE RESTRICT`, preventing precious organization from disappearing as an incidental asset-cleanup cascade. Because no absolute path or Preview reference enters this model, Media Root remapping, source unavailability, Preview cleanup, Catalog relocation, and validated whole-Catalog backup/recovery preserve organization automatically.

Issue #213 projects that foundation into the Browser without adding a second media workspace. `BrowserCollectionTreeModel` is presentation-only hierarchy state; every create/rename/delete/reparent/reorder operation still goes through `ICollectionOrganizationService`. Locations/Folders and Collections are independently collapsible sections of one vertically scrolling scope selector; collapse state is workspace presentation state, while `BrowserScopeSelection` owns the single active folder-or-Collection presentation and clears both model and realized WPF container selection in the inactive tree. A folder pointer/keyboard intent is recorded before `SelectedItemChanged`, allowing a real Collection→folder transition to start normal navigation while a passive delayed reveal is rejected; the shared outer ScrollViewer handles preview wheel input before either child TreeView can consume it. Hierarchy drag/drop carries only `BrowserCollectionNode` identity: center drops onto empty or populated Sets call the optimistic reparent contracts; edge drops choose an exact before/after position among the mixed Set/Collection sibling list, including an exact top-level position when moving out of a Set, and persist the complete mixed order through `ReorderHierarchyAsync`. The Collections header remains a convenient append-to-top-level target rather than the only way out of a Set; Browser asset drag/drop remains absent until #214. A TreeView-level `CollectionDropAdorner` draws a four-pixel accent insertion rule across the available row width or a restrained two-pixel whole-row outline for move-into-Set, keeping the target icon and text readable and independent of WPF's focus rectangle. Sort-by-name commands reorder the complete mixed sibling list durably rather than establishing a hidden display sort mode. Both Collection and Set creation share an explicit top-level-or-Set placement model. `BrowserCollectionScopeService` joins ordered membership to Catalog assets and Media Root availability without enumerating filesystem trees, then emits the same `MediaFolderEntry`/stable-asset identity vocabulary consumed by `BrowserGridModel`. The existing `BrowserQuery`, virtualized rows, selection, Preview/metadata hydration, Catalog asset-state projection, capability actions, and `PlayerViewerHost` therefore operate unchanged after membership establishes the candidate set. Missing or offline sources remain visible with retained Preview data and an explicit unavailable state; they are submitted to the existing derived-work scheduler as unavailable so no source probe is invented. Collection Sets remain non-selectable organizational nodes and never aggregate descendant Collection membership. Context-menu command targets remain separate from active-scope visual selection so a previously selected Set cannot receive a Collection command; genuine optimistic conflicts refresh the hierarchy and surface through Lightflow-styled, user-facing recovery UI.

Issue #214 adds membership gestures without changing either scope pipeline. Browser tile drags carry a dedicated `BrowserAssetDragPayload` containing stable `AssetId`s, never the `BrowserCollectionNode` hierarchy payload; starting a drag from any selected tile preserves and carries the full multi-selection, while a completed plain click still collapses it normally. Collection rows accept that payload as a copy-like logical membership operation while Collection Sets reject it. Context-menu and drag additions use the transactional `AddMembershipsAsync` batch per destination Collection and report success or an idempotent no-op through the nonblocking Browser status. Removal is an explicitly confirmed membership-only action, uses revision-checked `RemoveMembershipsAsync`, reloads only the active Catalog-backed Collection scope, and never performs source or Preview operations. Collection activation records discrete pointer or keyboard navigation intent; lingering tree focus is not treated as intent, so a hierarchy refresh cannot recursively reopen the selected Collection and strand its loading overlay. Collection scopes introduce `Manual` as a real `BrowserSortMode` and select it by default; explicit Name/date/type/size/duration choices continue through the shared query engine and disable manual tile reorder. Manual reorder is enabled only while search and filters are neutral, then persists the complete membership order through `ReorderMembershipsAsync`. Folder scopes do not expose Manual because it has no durable meaning there.
...
The global compact Jobs surface is a presentation/control projection directly over `GlobalExportScheduler.Jobs`. One scheduler
...
ordinary progress and terminal notifications cannot reopen a manually closed panel.
...
Queue controls use two compact rows at narrow widths. Clearing still dismisses eligible terminal JobIds only,
never recovery, reservations, or History. Actionable NeedsAttention Jobs remain visible; cancellation uses the
existing dark-shell confirmation and scheduler boundary.
...
The bottom status-bar Jobs action and compact Show all Jobs button route to the full Jobs destination.
The single Home right-panel toggle controls Inspector, Subclips, and Jobs. New accepted submissions select Jobs
and open the panel without navigating or rebuilding Home; ordinary progress/terminal updates cannot reopen it.
Returning from full Jobs restores the live Home and its panel preference. Global Jobs continues updating while
other surfaces are active or the panel is closed.
...
## Browser Grid and Details (#227)

`BrowserLayoutMode` chooses Grid or Details inside the existing Browser presentation. It is distinct from the retained Grid tile composition preference (`BrowserViewMode`: Preview/Info/Hybrid) and the Browser/Player presentation boundary. Details uses the same `BrowserGridModel`, ordered tiles, selection model, folder/Collection scope, Include Subfolders, typed query, filters, Catalog state, and Preview projection. Switching layout only changes the row template and grouping (one original tile per Details row); it does not navigate, enumerate, query a store, or request derived work.

`GridViewRowPresenter` and `GridViewHeaderRowPresenter` share one column collection. The existing recycling `VirtualizingStackPanel` owns vertical realization, and horizontal offsets synchronize headers with rows. Fixed-width rows are left-aligned so resizing does not center them away from their headers. Only realized rows instantiate cells, and only visible columns participate. Grouping still does linear work over the result set when switching layout; virtualization bounds visual/cell realization, not the in-memory result projection. No per-row Catalog reads, optional-column hydration, alternate thumbnail cache, or new metadata pipeline exists.

The accepted rows are 64 DIPs high with 54-DIP cached Previews. The ten default columns are Preview, Name, Rating, Flag, Capture Date, Media Type, Dimensions, Duration, Frame Rate, and File Size. Modified Date, Color Label, Camera, Lens, Color/LUT, Saved Range, Subclips, and Keywords are optional. Codec is not part of the existing Browser technical projection and is omitted. Markers (#226), raw/provider metadata columns, and arbitrary metadata editing are absent. Inspector retains deep inspection ownership.

Sortable headers call `ApplyBrowserQuery` through `BrowserDetails.Sort`; there are no WPF collection-view sort descriptions or table-local comparers. Name, Capture Date, Modified Date, Media Type, File Size, Duration, Rating, Flag, Dimensions, and Frame Rate share the typed `BrowserSortMode` vocabulary with Grid's Sort control. Missing values in the nullable sort modes stay last in both directions; existing Name/Key tie-breaking remains deterministic. Dimensions compare pixel area then width/height; frame rate uses the existing Browser canonicalization. Classification hydration includes Rating/Flag reordering, while technical metadata updates retain the existing coalesced resort timer.

Both templates use the same click, Ctrl-click, Shift-click, context selection, drag, and Open handlers. Shared keyboard navigation records a current AssetId. Layout switches preserve selection and Shift anchor; explicit query changes retain the established anchor-reset behavior, while live re-sorts remap an existing anchor by stable key. Double-click, Enter, and context Open enter the existing Player path. #111 captures the exact ordered shared projection for single-result-set or selected-subset review and retains its accepted return-selection behavior.
