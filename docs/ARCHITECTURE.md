...
Issue #124 adds recursive Browser scope — Include Subfolders — without disturbing the semantic pipeline `Location + scope mode → candidate media set → BrowserQuery → visible ordered result set` #108/#109 already established. `BrowserScopeMode` (`DirectFolder`/`IncludeSubfolders`) is the WPF-independent scope abstraction; `BrowserNavigationSession.ScopeMode` is the session's one standing mode (mirroring how sort/filter/search is a standing view preference, not a per-folder one — navigating to a different folder keeps whichever mode is active), and `SetScopeModeAsync` is the only way to change it. Toggling reloads the currently open folder as a same-folder `NavigationKind.Refresh` — reusing the exact generation/cancellation/latest-request-wins machinery every other navigation already has, so a rapid re-toggle or a folder change mid-scan safely supersedes obsolete recursive work — rather than as a `New` navigation, so it never pushes a back-stack entry for the folder that was already open. The abstraction intentionally stays this small: #124 does not build a generic scope-provider framework, but every consumer (navigation, monitoring relevance, persistence) is written against `BrowserScopeMode` rather than a bare bool, leaving room for later Favorites/Collections/Smart Collection/All Media/Recent scopes to extend the same seam.

Recursive discovery is entirely service-layer: `RecursiveMediaDiscoveryService` (`IRecursiveMediaDiscoveryService`) walks descendant folders by calling the *exact same* `IMediaFolderEnumerator.EnumerateAsync` + `IMediaDiscoveryRefreshService.RefreshAsync` pair per folder that a direct-folder view already uses for its one folder — there is no second reconciliation strategy, no parallel probing/thumbnail path, and no WPF-owned filesystem walk or watcher. The walk fans out with `Task.WhenAll` gated by a small `SemaphoreSlim` (default 4) around each folder's actual enumerate/reconcile pair, bounding concurrent filesystem/Catalog work regardless of how many folders exist, plus a total-visited-folder safety valve (default 20,000) that truncates pathologically large/looping trees gracefully (a noted diagnostic, not a failure) rather than walking unbounded. A descendant folder that individually fails (access denied, transient I/O, reconciliation failure) is skipped with a recorded diagnostic and its siblings still contribute — only a failure at the base folder itself fails the whole scope, matching "offline/unavailable roots remain non-destructive." Every visited folder's media files, reconciliation items, and (if scheduled) derived-work batch fold into one `RecursiveScopeResult`; `AggregateDerivedWorkBatch` combines the per-folder `IDerivedWorkBatch`s into the single `IDerivedWorkBatch` `BrowserFolderState.DerivedWork` already carries, so #108's existing grid/thumbnail/metadata hydration (`BrowserGridModel.ApplyAssetIdentities`, `BrowserDerivedWorkProjection`, `MainWindow.ApplyBrowserDerivedWorkResultsAsync`) needs no recursive-specific branch at all — it is simply handed a bigger batch. Because every folder's derived work still schedules onto the one shared `IDerivedWorkScheduler` instance (fixed worker pool, per-`AssetId` dedup) `MediaDiscoveryRefreshService` already uses, thousands of descendant assets entering scope cannot create unbounded Preview/metadata work merely by existing.

`BrowserFolderState.RecursiveMediaEntries` (populated only in `IncludeSubfolders` mode) is the flattened, files-only candidate set across every visited folder; `Entries` (this folder's own direct listing) is left untouched and keeps feeding `BrowserTreeModel.Synchronize`'s direct-child folder listing, since the Locations tree always reflects direct children regardless of scope mode — recursion is a media-canvas concern, not a navigation-tree concern. `MainWindow.ApplyBrowserState` populates the grid from `state.RecursiveMediaEntries ?? directFiles`, so `BrowserGridModel`/`BrowserQueryEngine`/`BrowserQuery` remain completely unaware recursion exists; `IsPresentable`'s existing #108 filtering (folders, standalone audio, and unsupported files excluded) and #109's full sort/filter/search/status behavior apply identically to the larger recursive set with no special-case query code.

Scope changes and query changes are deliberately asymmetric. `_browserQueryScope` (keyed on `RootId`+`RelativeFolder` only, unchanged from #109) still decides whether `BrowserQuery` resets — toggling Include Subfolders never touches it, so search/filter/sort survive a scope change exactly as #124 requires even though the candidate set itself changed. Selection uses a separate identity, `_browserScopeIdentity` (`RootId`+`RelativeFolder`+`BrowserScopeMode`): any change there — folder navigation *or* toggling Include Subfolders, even while staying in the same folder — calls `BrowserGridModel.ClearSelection()` before `Populate`, so an asset outside the newly active scope can never survive invisibly selected (relevant for future #75 capability actions). A true same-scope refresh (explicit Refresh, a relevant monitoring event) leaves `_browserScopeIdentity` unchanged and therefore preserves selection through `Populate`'s existing key-based retention, exactly like #108's non-destructive refresh.

Monitoring extends without a new watcher. `BrowserScope.IsWithinFolderScope` answers "is this monitoring event's folder inside that base recursive scope" (equal, a descendant, or the base is the Media Root itself); direct mode keeps #108's exact-folder match unchanged. Because a burst of filesystem activity across many descendant folders can raise several `IMediaRootMonitoringService.FolderRefreshed` events in quick succession, and each one would otherwise re-run the entire recursive scope, `MainWindow` coalesces relevant recursive events through a dedicated debounce timer into one authoritative refresh rather than one per event — a refresh-storm guard layered on top of #101's own per-folder debounce, using the same debounce convention. Either way a watcher event remains a hint only; explicit Refresh stays authoritative.

Persistence integrates with #121's existing seam rather than adding a new one: `WorkspaceBrowserLocationState.IncludeSubfolders` is Browser workspace scope state (not `BrowserQuery`, Catalog, or Preview state), written alongside the remembered `RootId`/`RelativeFolder` at the same point `ApplyBrowserState` already persists the location, and normalized to `false` for any missing/legacy document. `MainWindow.RestoreBrowserLocationAsync` applies the saved mode via `SetScopeModeAsync` *before* calling `BrowserLocationRestoration.RestoreAsync` — since no location is open yet, this only sets the session's field, and restoration then loads through the exact same direct/recursive branch an interactive toggle would use, never a separate startup-only recursive path. Media selection remains outside the persisted model entirely, unchanged from #121.

A follow-up UX revision (still #124) moves Include Subfolders into the media toolbar, immediately before the All/Images/RAW/Video group — scope narrows the candidate set first, so it now reads left-to-right as *scope → media type → search → filter → sort*, matching that pipeline. It keeps its own `BrowserScopeToggleButtonStyle` (a two-folder icon composed entirely from the same verified folder glyph the Locations tree already renders, plus a text label) rather than the segmented group's or Filter ▾'s style, and a wider gap than the toolbar's other spacing is the only thing separating it from the media-type controls — enough to avoid reading as one of those segments without reading as unrelated to them.

This revision also turns the recursive loading indicator from a decorative indeterminate accent line into a truthful progress bar. `RecursiveMediaDiscoveryService.DiscoverAsync` accepts an optional `IProgress<RecursiveScopeProgress>`; `RecursiveScopeProgress` (`FoldersDiscovered`, `FoldersVisited`) is deliberately not a final total — a true total is unknowable without a separate traversal this service does not perform — but an honest "done vs. currently known" pair computed from the same counters the walk already maintains (`WalkState.TryReserveFolder`/`MarkVisited`), reported once per folder discovery and once per folder completion with no extra filesystem work. `BrowserNavigationSession.RecursiveScopeProgressChanged` re-raises reports for whatever recursive walk is current, gated by the exact same generation check `Commit` already uses, so a report from a walk superseded by a newer toggle/navigation is silently dropped rather than ever reaching a subscriber — no separate staleness check is needed at the `MainWindow` layer. The session hands the discovery service a plain `SynchronousProgress<T>` rather than `System.Progress<T>`: `Progress<T>` posts its callback through the `SynchronizationContext` captured at construction — asynchronously, via the thread pool, whenever none is present — which made exactly when a report becomes observable non-deterministic; `MainWindow` marshals onward to the UI thread itself, explicitly, exactly like every other cross-thread signal in this file, so nothing depends on `Progress<T>`'s own marshaling. `MainWindow.ApplyRecursiveScopeLoadingProgress` stays indeterminate until `FoldersDiscovered` grows past the trivial single-folder case (so a small recursive scope never flashes a near-instant, uninformative "1 of 1"), then sets `Maximum`/`Value` directly from the reported counts — never a fabricated percentage. Direct-mode navigation never receives progress events at all, so its indicator stays indeterminate throughout, exactly as before.

The same revision adds a continuous outline in the Locations tree around the active recursive-scope group — the selected base folder plus its currently visible descendant rows — while exactly one row remains individually selected, matching #107/#108's existing selection model unchanged. `BrowserTreeModel.LastVisibleDescendant` is a pure, static function over already-expanded `BrowserTreeNode` state: it follows "last real (non-placeholder) child, recursively" while `IsExpanded` holds, which is always the true bottom-most visible row for a standard top-to-bottom TreeView render order; a collapsed branch or an unmaterialized lazy-load placeholder simply stops the descent, so nothing is force-expanded or force-materialized merely to draw the outline. `MainWindow.SyncBrowserScopeOutline` locates the selected node's and that function's result's `TreeViewItem` containers (via the existing `FindBrowserTreeItem`), then positions one `Border` — `BrowserScopeOutline`, `IsHitTestVisible="False"`, styled with the same `ShellFocusBrush` used for keyboard-focus outlines elsewhere — purely via `Margin`/`Height` computed from `TranslatePoint`, deferred to `DispatcherPriority.Loaded` exactly like the pre-existing `BringBrowserTreeNodeIntoView`. Scope membership is never encoded as `IsSelected`, and this method never mutates selection at all — it is a presentation-only overlay layered beside (not merged into) `BrowserGridTile`'s and the tree row's own selection concerns. `BrowserScopeOutline` is declared as a sibling of `BrowserFolderTree` inside one wrapping `Grid`, and that `Grid` — not the `TreeView` directly — is `BrowserFolderScrollViewer`'s single content child, so the outline scrolls in lockstep with the rows it outlines with no manual scroll-offset math, and is clipped to the viewport for free by the `ScrollViewer`'s own content presenter. `SyncBrowserScopeOutline` is called everywhere the visible scope group can change: `ApplyBrowserState` (navigation/mode toggle/failure), a new `Collapsed` `EventSetter` alongside the pre-existing `Expanded` one, and after `RevealBrowserTreeAncestorsAsync` finishes materializing ancestors — always hidden outright in direct mode or when nothing is selected.
