...
The Player's contextual Subclips surface is a restrained right-side panel inside the Player host, not permanent shell navigation. Its fixed compact width yields space to media, uses the shared panel/card/focus vocabulary, and establishes the compatible location for later contextual Inspector/Color work without speculatively building that architecture. Subclip cards pair a quiet poster with name, exact range/duration text, clear selection, and keyboard-focusable rename/order/delete actions; missing posters retain a deterministic media glyph rather than blocking interaction.
