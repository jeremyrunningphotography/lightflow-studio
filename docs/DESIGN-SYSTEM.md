...
The Browser owns a 260-pixel left navigation panel for configured Media Roots and uses the remaining width for the current logical folder. Its compact Back/Forward/Up/Refresh toolbar is application chrome, while folder/file rows are deliberately quiet and information-first. Online state is reinforced with text as well as color; unavailable roots remain selectable so the workspace can explain what happened. Loading and empty/error states occupy the media canvas without replacing the user's navigation context.

Issue #107's entry list is intentionally a basic navigation surface, not a preview grid. Thumbnails, selection, sorting/filtering/search, Player/Viewer, Inspector/Color, and Browser-to-capability handoff arrive in later Browser slices without changing the shell or Media Root ownership of the left edge.
