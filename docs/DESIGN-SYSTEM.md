...
The Browser owns a 260-pixel filesystem-oriented Locations panel and uses the remaining width for the current folder. Familiar drives and mapped/removable storage are primary entry points; managed Media Roots appear as pinned libraries rather than setup prerequisites. Its compact Back/Forward/Up/Refresh toolbar and editable path field are application chrome, while folder/file rows are deliberately quiet and information-first. Online state is reinforced with text as well as color; unavailable storage remains visible so the workspace can explain what happened. Loading and empty/error states occupy the media canvas without replacing navigation context.

Issue #107's entry list is intentionally a basic navigation surface, not a preview grid or full Windows Shell namespace. Thumbnails, selection, sorting/filtering/search, Player/Viewer, Inspector/Color, and Browser-to-capability handoff arrive in later Browser slices without changing the shell or Browser ownership of the left edge.
