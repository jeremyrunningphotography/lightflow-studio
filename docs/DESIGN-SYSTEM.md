...
Issue #109 adds one compact toolbar row directly above the grid, never inside the Locations sidebar: a filename/path search box, a `Filter ▾` control, a sort `ComboBox`, a single icon-only sort-direction toggle, and a muted status line sharing the row's remaining width. It reuses the same panel surface and restrained, icon-forward visual language as the folder-navigation toolbar above it (icon-only Back/Forward/Up/Refresh buttons already establish that a ToolTip and `AutomationProperties.Name`, not a visible text label, is this workspace's convention for a self-evident control) rather than introducing new chip/segmented-button styling for the everyday row. The status line stays a single line of restrained, muted text — visible/total counts, selection count and size, and Preview-generation activity share one line via a middle dot separator rather than competing badges — and disappears into the toolbar's normal quiet state the moment nothing is happening.

`Filter ▾` uses progressive disclosure rather than a permanent row of one-off filter controls: it opens a small dark popup of checkboxes for the currently available predicates (media type today). An active predicate then appears as a compact removable chip — label plus a small "×" — in a second row that exists only while at least one filter is active; with no filters active, that row's height and visual weight are gone entirely rather than sitting empty, so ordinary browsing stays a single quiet toolbar line. Chips use the same restrained selection-surface and divider-brush treatment as other quiet dark UI here, not a bright or saturated accent, keeping several active predicates from reading as noise. The remove control on a chip is a real button (keyboard-reachable and -activatable), not a bare clickable glyph.
