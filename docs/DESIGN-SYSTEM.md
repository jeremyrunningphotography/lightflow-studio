...
Issue #109 adds one compact toolbar row directly above the grid, never inside the Locations sidebar: a persistent All/Images/RAW/Video quick-filter row, a filename/path search box, a `Filter ▾` control, a sort `ComboBox`, and a single icon-only sort-direction toggle. It reuses the same panel surface and restrained, icon-forward visual language as the folder-navigation toolbar above it (icon-only Back/Forward/Up/Refresh buttons already establish that a ToolTip and `AutomationProperties.Name`, not a visible text label, is this workspace's convention for a self-evident control) rather than introducing new chip styling for the everyday row. The status line — visible/total counts, selection count and size, and Preview-generation activity sharing one line via a middle dot separator rather than competing badges — lives in its own quiet bar beneath the grid instead of the toolbar, so it stays with the media it describes and the toolbar itself stays a single row of controls with nothing competing for width.

The quick-filter row is four `ToggleButton`s sharing one compact style (a selection-brush background/border only on the checked state, via a style trigger rather than a locally-set brush); "All" clears the media-type facet entirely, while Images/RAW/Video each pick that one value exclusively — the everyday single-value case in one click. `Filter ▾` sits alongside it for the stackable/compound case: it uses progressive disclosure rather than a permanent row of one-off filter controls, opening a small dark popup of checkboxes for the currently available predicates (media type today, with a muted caption noting more fields arrive over time) plus grouped placeholders for fields not yet implemented. An active predicate then appears as a compact removable chip — label plus a small "×" — in a second row that exists only while at least one filter is active; with no filters active, that row's height and visual weight are gone entirely rather than sitting empty, so ordinary browsing stays a single quiet toolbar line. Chips use the same restrained selection-surface and divider-brush treatment as other quiet dark UI here, not a bright or saturated accent, keeping several active predicates from reading as noise. The remove control on a chip is a real button (keyboard-reachable and -activatable), not a bare clickable glyph. The quick buttons and `Filter ▾`'s checkboxes both write the same underlying facet and stay mutually consistent — checking "Video" via either control checks the other's corresponding control too.
