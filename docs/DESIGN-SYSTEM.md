...
The Player's contextual Subclips surface is a restrained right-side drawer inside the Player host, not permanent shell navigation. Its right-edge pull exists only for a Catalog-backed video in Player; the shell coordinates it mutually exclusively with Jobs, and its compact media-consuming column collapses completely while closed or retired. It uses the shared panel/card/focus vocabulary and establishes the compatible location for later contextual Inspector/Color work without speculatively building that architecture. Subclip cards pair a quiet poster with readable semantic name, exact range/duration text, desktop multi-selection, and keyboard-focusable rename/order/delete actions; missing posters retain a deterministic media glyph rather than blocking interaction.
