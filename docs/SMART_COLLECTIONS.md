...
Authoring metadata distinguishes finite choices, observed facets, structured dimensions/fps/time, dates, rating, state and arbitrary text. Resolution accepts exact positive pixel dimensions without requiring a current match. Frame-rate suggestions reuse `MediaFrameRate.Canonical` plus observed values; positive custom input uses `BrowserFrameRate.Canonicalize`. Duration accepts time input for the existing inclusive minimum only. Boolean state operators have no redundant third value input; ReviewRange is presented as In/Out Range / is set or is not set. Camera/Lens/Keywords are unavailable for new rows when their known vocabulary is empty, while existing selections stay editable. See the [complete editor inventory](validation/smart-filter-editor-inventory-217.md). These input conversions do not add evaluation semantics or change Browser contextual facets.
