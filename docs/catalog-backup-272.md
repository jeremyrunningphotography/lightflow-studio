...
## Final reconciliation review (2026-09-22)

Accepted local checkpoint: `e9e4ed7039ee6337f9cb0d5efb72d39ba9dade8b`. Current-main base: `686b9bf26b75eda1cc840bbf83cebb28390625ba` (Settings #304, Jobs #305, Visual Index #301, preceding rotation/polish).

Reconciliation retains main's Settings categories, preference-only `AppSettings.ApplyPreferences`, separate Export defaults, Jobs cleanup/queue authority, and package-smoke helper. The accepted backup controls move into the current Catalog & Previews / Backup & Recovery card; destination, confirmation, progress, cancellation and retention semantics are unchanged. Backup itself never enters `JobsAdmission` or the Jobs runtime.

With #297's shared scheduler, a queued file/Premiere job has not started a Catalog mutation. It first obtains its existing Jobs slot, then acquires Catalog operation admission. Its complete executing operation remains admitted through all transactions, file reconciliation / presentation publication, or Premiere callbacks. Acquiring Catalog admission before a paused/capacity-limited Jobs slot would invert ownership against another scheduled worker waiting to publish a Catalog observation. No generic Jobs scheduling rule is changed. Deterministic integration tests cover paused jobs, continued execution after reopen, and draining final multi-transaction publication/HTTP receipt.

Final source review searched all Catalog SQL write sites and rechecked the inventory above. All runtime data writes reach the storage-owned lifecycle; normal reads and connection-local SQLite runtime PRAGMAs remain outside it. Schema migrations, initial identity/schema creation, startup recovery, and staged-copy validation retain lifecycle-exclusive ownership. PreviewPersistence writes its separate rebuildable database. Profile/workspace/Jobs JSON remains outside the Catalog snapshot. Runtime Catalog restore/relocation is admitted and drained before an exit snapshot.

Race review: closing admission, counting admission/release and checking the drained state share one lock. Live nested and explicit HTTP continuation leases are counted independently; released owners cannot bypass closure. Exceptions/cancellation dispose leases. Cancelled drains and unsuccessful snapshots reopen; successful exit retains quiescence until shutdown; disposal wakes waiters. Deterministic tests exercise concurrent admission/drain, multi-transaction operations, cancellation/failure, stale contexts/capabilities and repeated reopen cycles. Real SQLite tests verify final accepted rows, not only structural validity.

Acceptance classification: behaviorally clean integration with current main. The Settings move is the explicitly permitted integration into #291's architecture; accepted #272 product behavior and logical-operation drain guarantees remain intact. No additional hands-on cycle is required by this reconciliation. Physical network/removable-device tests remain unavailable; task-owned fixtures cover unavailable/write-failure destinations and isolated containment. The policy is boolean enabled/disabled only; frequency and due/not-due cases are not applicable.
